...
export default function ProfileHeader({ user = {}, loading = false, onEdit, onBusinessDetails, onOpenSite }) {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) { navigator.share({ title: user?.fullname, url }); }
    ...
    <div className="w-full flex items-start gap-4 sm:gap-6">
      <div className="w-full flex flex-col items-center sm:flex-row sm:items-start gap-5">
        ...
        {loading
          ...
          : (
            <button onClick={onOpenSite} title="View public profile"
              ...
              <img src={avatarSrc} alt={user?.fullname} onError={e => { e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.fullname||"U")}&background=5323DC&color=fff&size=150`; }}
        ...
        <div className="w-full flex flex-col items-center sm:items-start gap-2 sm:pr-5">
          ...
          {loading ? (
            <><Skeleton className="h-7 w-48" /><Skeleton className="h-4 w-64 mt-1" /><Skeleton className="h-4 w-40 mt-1" /></>
          ) : (
            <>
              {/* Name + verified */}
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-2xl font-semibold text-[#0F1724]">{user?.fullname || "—"}</h1>
                {user?.status === "Active" && <MdOutlineVerified className="text-violet-600 w-6 h-6 shrink-0" />}
              </div>

              {/* Role + username + website */}
              <p className="text-sm text-[#9CA3AF] flex items-center gap-2 flex-wrap justify-center sm:justify-start">
                {user?.role && <span className="font-medium text-gray-600">{user.role}</span>}
                {user?.username && <><span>·</span><span>@{user.username}</span></>}
                {user?.contact && (
                  <button onClick={onOpenSite} className="flex items-center gap-1 text-[#5323DC] hover:underline">
              ...
              {user?.referral && (
                <p className="text-sm text-gray-500">
                  Referral: <span className="font-bold text-[#5323DC]">{user.referral}</span>
                </p>
              )}
            </>
          )}
          ...
          <div className="flex gap-6 sm:gap-10 mt-2 sm:mt-4">
            {[["—", "Followers"], ["—", "Following"], ["—", "Posts"]].map(([v, l]) => (
          ...
          <div className="w-full flex gap-3 justify-center sm:justify-start pt-3 flex-wrap">
            ...
            <button onClick={onBusinessDetails}
              className="px-4 sm:px-5 py-2 rounded-lg bg-violet-50 text-violet-600 text-sm font-medium hover:bg-violet-100 transition-colors active:scale-95">
              Business Details
            </button>
