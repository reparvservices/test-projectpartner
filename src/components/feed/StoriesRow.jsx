    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95" onClick={onClose}>
      ...
      >
        ...
        <div className="absolute top-3 inset-x-3 flex gap-1 z-20">
          {group.stories.map((_, i) => (
            <div key={i} className="flex-1 h-[3px] rounded-full bg-white/30 overflow-hidden">
              ...
              {i === sIdx && (
                <div key={`${gIdx}-${sIdx}`} className="h-full bg-white rounded-full"
        ...
        <div className="absolute top-8 inset-x-3 flex items-center gap-2.5 z-20">
          <Avatar src={group.author_image} name={group.author_name} size={34} ring ringHex="#fff" />
          <div>
            <p className="text-white text-sm font-semibold leading-none">{group.author_name}</p>
        ...
        {story.caption && (
          <div className="absolute bottom-16 inset-x-0 px-5 py-3 bg-gradient-to-t from-black/70 to-transparent">
            <p className="text-white text-sm text-center font-medium">{story.caption}</p>
          </div>
        )}

        {/* Nav zones */}
        <button className="absolute left-0 top-0 bottom-0 w-1/3 opacity-0" onClick={() => onAdvance(gIdx, sIdx - 2)} />
        <button className="absolute right-0 top-0 bottom-0 w-1/3 opacity-0" onClick={() => onAdvance(gIdx, sIdx)} />
        ...
        <div className="absolute bottom-4 inset-x-4">
          <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-4 py-2.5">
            <input placeholder="Reply to story…" className="flex-1 bg-transparent text-white text-sm placeholder-white/50 outline-none" />
    ...
    try {
      ...
      await api("/stories", {
        ...
        body: JSON.stringify({
          ...
          media_url: url, media_type: preview.type,
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          ...
          <h2 className="text-sm font-bold text-gray-900 tracking-wide">New Story</h2>
          {step === "preview"
            ? <button onClick={() => setStep("caption")} className="text-violet-600 text-sm font-bold">Next →</button>
            : step === "caption"
        ...
        {step === "pick" && (
          <div className="flex flex-col items-center justify-center py-14 gap-5 px-8 bg-gradient-to-b from-violet-50 to-white">
            ...
            <div className="text-center">
              <p className="text-sm font-semibold text-gray-800">Add to your story</p>
              <p className="text-xs text-gray-400 mt-1">Photos and videos disappear after 24 hours</p>
            </div>
        ...
        {step === "preview" && preview && (
          <div className="relative bg-black" style={{ aspectRatio: "9/16", maxHeight: 440 }}>
            {preview.type === "video"
              ? <video src={preview.url} autoPlay muted playsInline className="w-full h-full object-cover" />
        ...
        {step === "caption" && preview && (
          <div>
            <div className="relative bg-black" style={{ height: 240 }}>
              {preview.type === "video"
                ? <video src={preview.url} muted className="w-full h-full object-cover opacity-70" />
                : <img src={preview.url} alt="" className="w-full h-full object-cover opacity-70" />}
              ...
              {uploading && (
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center gap-3">
                  <div className="relative w-16 h-16 flex items-center justify-center">
                    ...
                    <span className="text-white text-sm font-bold z-10">{progress}%</span>
                  </div>
...
export default function StoriesRow({ actor }) {
  ...
  const load = () =>
    api(`/stories?user_id=${actor.user_id}&user_role=${encodeURIComponent(actor.user_role)}`)
  ...
  const markViewed = (id) =>
    api(`/stories/${id}/view`, {
      method: "POST", body: JSON.stringify({ user_id: actor.user_id, user_role: actor.user_role }),
    }).catch(() => {});
  ...
  return (
    <>
      <div className="flex gap-4 overflow-x-auto scrollbar-hide py-1 px-1">
        ...
        <div className="flex flex-col items-center gap-1.5 shrink-0">
          <button
            ...
            style={{ background: "linear-gradient(145deg,#7c3aed15,#ec489915)", border: "2px dashed #7c3aed60" }}
          >
            <div className="w-6 h-6 rounded-full bg-violet-600 flex items-center justify-center absolute -bottom-0.5 -right-0.5 border-2 border-white">
              <Icon name="plus" size={12} className="text-white" sw={3} />
            </div>
            <Avatar src={getImageURI(actor.user?.userImage)} name={actor.user?.fullname || "Me"} size={52} />
          </button>
          <span className="text-[11px] text-gray-500 font-medium truncate max-w-[60px] text-center">Your Story</span>
        </div>
        ...
        {groups.map((grp, gIdx) => (
          ...
          >
            <div className="relative">
              <Avatar
                src={grp.author_image} name={grp.author_name} size={60}
                ring ringGradient={grp.has_unseen}
      ...
      {active && (
        <StoryViewer groups={groups} gIdx={active.gIdx} sIdx={active.sIdx}
          onAdvance={advance}
          onClose={() => { clearTimeout(timerRef.current); setActive(null); }} />
      )}
      {showUpload && (
        <StoryUploadModal actor={actor} onClose={() => setShowUpload(false)} onUploaded={load} />
      )}
    </>
  );
}